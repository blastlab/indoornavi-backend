...
FROM maven:3.3-jdk-8 AS compiler
...
RUN cp /app/src/main/webapp/env.js /app/deployments/
...
COPY --from=compiler /app/deployments /opt/jboss/wildfly/standalone/deployments
