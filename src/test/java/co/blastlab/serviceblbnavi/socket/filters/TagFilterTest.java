...
public class TagFilterTest {
	...
	public void updateAndFilter() throws Exception {
		...
		assertThat("After first filtration tag with id 1 should be filtered", filtered.size(), is(0));

		// turn tag with id 1 activity back to active
		tagFilter.update(mockedSession, 1);
		filtered = tagFilter.filter(Collections.singleton(mockedSession), 1);

		assertThat("After second filtration tag with id 1 should NOT be filtered", filtered.size(), is(1));
	}
